...
void BaseCharacter::Update()
{
	if( this->yVelocity != 0 )
	{
		if( (BaseCharacter::_Position.y + this->yVelocity > 0) && (BaseCharacter::_Position.y + this->yVelocity < Gfx.screen->h - 100) )
			BaseCharacter::_Position.y += this->yVelocity;
	}
	if( this->xVelocity != 0 )
	{
		if( (BaseCharacter::_Position.x + this->xVelocity > 0) && (BaseCharacter::_Position.x + this->xVelocity < Gfx.screen->w - 128) )
			BaseCharacter::_Position.x += this->xVelocity;
	}
	...
	if( this->isHit )
	{
		...
		if( BCPlayer._HealthPoints > 0 )
		{
			--BCPlayer._HealthPoints;
...
void BaseCharacter::Reset()
{
	...
	//this->xPos = Xpos;
	//this->yPos = Ypos;
	this->_Position.x = GROUND_X;
	this->_Position.y = GROUND_Y;
	this->_Position.h = demonHEIGHT;
	this->_Position.w = demonWIDTH;
